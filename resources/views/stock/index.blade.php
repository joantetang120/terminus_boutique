<x-app-layout title="Stock">
        ...
        <div class="page-header-actions">
            ...
            @can('stock.create')
                <div style="display:flex;gap:8px;">
                    <button class="btn btn-success btn-sm" x-data @click="$dispatch('open-entry-modal')">+ Entrée
                        stock</button>
                    <button class="btn btn-warning btn-sm" style="background:#E67E22;color:white;" x-data
                        @click="$dispatch('open-exit-modal')">+ Sortie manuelle</button>
                </div>
            @endcan
        </div>
    </div>
    ...
    <div class="table-wrapper">
                            ...
                            @if ($movement->product)
                                @if ($movement->input_unit && $movement->input_unit !== $movement->product->unit)
                                    <span title="Converti: {{ $movement->quantity }} {{ $movement->product->unit }}">
                                        {{ number_format($movement->input_quantity, 0, ',', ' ') }}
                                        {{ $movement->input_unit }}
                                        <small
                                            style="color:#64748B;">({{ number_format($movement->quantity, 0, ',', ' ') }}
                                            {{ $movement->product->unit }})</small>
                                    </span>
                                @else
        ...
        <div class="pagination-wrapper">
            ...
            <div class="pagination-info">
                Affichage {{ $movements->firstItem() ?? 0 }} - {{ $movements->lastItem() ?? 0 }} sur
                {{ $movements->total() }} mouvements
            </div>
        </div>
    </div>

    {{-- Entry Modal --}}
    <div x-data="{ open: false }" @open-entry-modal.window="open = true" x-show="open" x-cloak class="modal-backdrop"
        style="display:none;">
        <div class="modal" @click.away="open = false">
            <div class="modal-header">
                <h3>Entrée de stock</h3>
                <button @click="open = false"
                    style="background:none;border:none;cursor:pointer;font-size:1.25rem;">&times;</button>
            </div>
                ...
                <div class="modal-body">
                    ...
                    <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;">
                        <div class="form-group">
                            ...
                            <input class="form-input" type="number" id="entry_qty" name="quantity" min="1"
                                step="1" required onchange="calculateEntryCost()">
                        </div>
                        <div class="form-group">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="form-group">
                            ...
                            <input class="form-input" type="number" id="entry_unit_cost" name="unit_cost"
                                min="0" step="0.01" onchange="calculateEntryCost()">
                            <small class="field-note" id="entry_purchase_price_hint" style="color:#64748B;">Prix par
                                défaut: --</small>
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="entry_total_cost">Coût total (FCFA)</label>
                            <input class="form-input" type="number" id="entry_total_cost" name="total_cost"
                                min="0" step="0.01" readonly>
                        </div>
                    </div>
                    <div id="entry_conversion_info"
                        style="display:none;padding:8px 12px;background:#F0FDF4;border:1px solid #86EFAC;border-radius:6px;margin-bottom:12px;font-size:0.75rem;color:#166534;">
                    </div>
                    <div class="form-group">
    ...
    {{-- Exit Modal --}}
    <div x-data="{ open: false }" @open-exit-modal.window="open = true" x-show="open" x-cloak class="modal-backdrop"
        style="display:none;">
        <div class="modal" @click.away="open = false">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select class="form-select" id="exit_product" name="product_id" required
                            onchange="updateExitUnitOptions()">
                            <option value="">Sélectionner...</option>
                            @foreach ($products as $product)
                                @php
                                    $saleConversions = $product->saleConversions
                                        ->map(fn($c) => ['unit' => $c->unit, 'rate' => $c->conversion_rate])
                                        ->toArray();
                                    // Include legacy sale unit if exists and not already in conversions
                                    if (
                                        $product->sale_unit &&
                                        !collect($saleConversions)->first(fn($c) => $c['unit'] === $product->sale_unit)
                                    ) {
                                        $saleConversions[] = [
                                            'unit' => $product->sale_unit,
                                            'rate' => $product->sale_conversion_rate,
                                        ];
                                    }
                                @endphp
                                <option value="{{ $product->id }}" data-unit="{{ $product->unit }}"
                    ...
                    <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;">
                        <div class="form-group">
                            ...
                            <input class="form-input" type="number" id="exit_qty" name="quantity" min="1"
                                step="1" required>
                        </div>
                        <div class="form-group">
                    ...
                    <div id="exit_stock_info"
                        style="padding:8px 12px;background:#F1F5F9;border-radius:6px;margin-bottom:12px;font-size:0.75rem;color:#475569;">
                        Sélectionnez un produit
                    </div>
                    <div id="exit_conversion_info"
                        style="display:none;padding:8px 12px;background:#FEF3C7;border:1px solid #FCD34D;border-radius:6px;margin-bottom:12px;font-size:0.75rem;color:#92400E;">
                    </div>
                    <div class="form-group">
    ...
    <div x-data="{ open: false, id: null, product: '' }"
        ...
        <div class="modal" @click.away="open = false">
            ...
            <form :action="'/stock/' + id + '/cancel'" method="POST">
                @csrf
                <div class="modal-body">
                    <p style="margin-bottom:16px;color:#64748B;">Annuler le mouvement pour <strong
                            x-text="product"></strong> ?</p>
                    <div class="form-group">

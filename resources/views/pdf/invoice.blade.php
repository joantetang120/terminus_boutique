...
<body>
    <div class="receipt">
        {{-- Logo --}}
        <div class="logo">
            <img src="{{ public_path('img/logo-blue.png') }}" alt="{{ config('app.name') }}">
        </div>
        ...
        @if($invoice->payments->count() > 0)
            <div class="payment-info">
                ...
                @foreach($invoice->payments as $payment)
                    {{ $payment->payment_date->format('d/m/Y') }} - {{ number_format($payment->amount, 0, ',', ' ') }} FCFA {{ $payment->payment_method ? '('.$payment->payment_method.')' : '' }}<br>
                @endforeach
            </div>
        @endif
